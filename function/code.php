...
if ((strpos($message, "/key") === 0) || (strpos($message, "!code") === 0) || (strpos($message, ".code") === 0)) {
    $owners = file_get_contents('Database/owner.txt');
    $admins = explode("\n", $owners);
    if (!in_array($userId, $admins)) {
        sendMessage($chatId, "⚠️ 𝒀𝒐𝒖 𝑨𝒓𝒆 𝑵𝒐𝒕 𝑨𝒏 𝑨𝒅𝒎𝒊𝒏 !", $messageId);
    } else {
        ...
        if ($command == ' ' || $command == '') {
            ...
            if (count($cmdExplode) != 2) {
                sendMessage($chatId, "⚠️ 𝑰𝒏𝒗𝒂𝒍𝒊𝒅 𝑪𝒐𝒎𝒎𝒂𝒏𝒅 𝑭𝒐𝒓𝒎𝒆𝒕..                                           /code {expiry_days}-{amount_of_codes}", $messageId);
                exit;
            }
        ...
        while ($amountOfCodes > 0) {
            $rnds = 'MIKE-' . random_strings(4) . '-' . random_strings(4) . '-' . 'SUKUNA';
            $credt[] = $rnds;
            $amountOfCodes = $amountOfCodes - 2;
        }
